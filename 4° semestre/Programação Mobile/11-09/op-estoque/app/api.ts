...
import IOperacaoDetalhePost from "./interfaces/iOperacaoDetalhePost";
import IOperacao from "./interfaces/iOperacao";
import IOperacaoRequest from "./interfaces/iOperacaoRequest";
...
export async function ListarOperacoesEstoque(): Promise<IOperacaoRequest[]> {
    try {
        ...
        const data: IOperacaoRequest[] = await response.json();
        return data;

    } catch (error) {
        console.error("Falha ao buscar operações de estoque:", error);
        return [];
    }
}

export async function SalvarOperacoesEstoque(motivo: string, entradaSaida: string, detalhes: IOperacaoDetalhePost[]): Promise<IOperacao[]> {
    try {
        const payload = {
            motivo,
            entradaSaida: entradaSaida?.toUpperCase() ?? "",
            detalhes,
        };
        ...
        const data: IOperacao[] = await response.json();
        return data;

    } catch (error) {
        console.error("Falha ao salvar operações de estoque:", error);
        return [];
    }
}