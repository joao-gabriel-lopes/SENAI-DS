...
import IProdutoLista from "@/app/interfaces/iProdutoLista";
...
export default function PesquisaProdutos() {
  ...
  const [produtos, setProdutos] = useState<IProdutoLista[]>([])
    ...
    <>
      ...
      <section className={cardStyles.containerCardProduto}>
        ...
        <div className={cardStyles.cardProdutoTitulo}>
          ...
          <p className={cardStyles.cardDescricao}>Descricao:</p>
...

        {
          produtos.map(produto => (
            <div key={produto.id?.toString()} onClick={() => router.push(`/telas/produto/cadastro?id=${produto.id}`)} className={cardStyles.cardProduto}>
              <p className={cardStyles.cardNome}>{produto.produtoNome}</p>
              <p className={cardStyles.cardCategoria}>{produto.categoriaNome}</p>
              <p className={cardStyles.cardDescricao}>{produto.descricao}</p>
              <p className={cardStyles.cardQuantidade}>{produto.quantidadeAtual.toFixed()} {produto.unidadeMedida}</p>
            </div>
          ))
        }
